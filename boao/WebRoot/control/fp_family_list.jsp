...
<script type="text/javascript">
		function del(id){
			...
			if(result){
				window.location.replace("action!toDeleteFamily.do?id="+id);
			}
			return;
		}
	</script>
...
<body>
	<table class="tbl" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td class="tbl_middle">名人/家族管理--》名人家族</td>
					...
					<table width="800" border="1" align="center" cellpadding="0" cellspacing="0">
						<tr>
							<td height="30" colspan="7" align="left" valign="middle"><span class="STYLE4">当前数据语言：</span><strong>${FP_ENCODING}</strong> <c:if test="${FP_ENCODING == 'cn'}">
									<select name="encoding" onchange="SendRedirect(this.value);">
										<option value="action!toFamily.do?encoding=cn" selected="selected">中文[CN]</option>
										<option value="action!toFamily.do?encoding=en">英文[EN]</option>
									</select>
								</c:if> <c:if test="${FP_ENCODING == 'en'}">
									<select name="encoding" onchange="SendRedirect(this.value);">
										<option value="action!toFamily.do?encoding=en" selected="selected">英文[EN]</option>
										<option value="action!toFamily.do?encoding=cn">中文[CN]</option>
									</select>
								</c:if></td>
						</tr>
						<tr>
							<td width="50" height="30" align="center" valign="middle" bgcolor="#EAEAEA">编号Id</td>
							<td width="230" height="30" align="center" valign="middle" bgcolor="#EAEAEA">家族名称</td>
						...
						<tr>
							<td height="40" colspan="7" align="left" valign="middle">
								<p>
									&nbsp;当前语种下共有记录<strong>${fn:length(FP_FAMILY_TOTAL_LIST)}</strong>条， 当前是第<strong>${FP_FAMILY_PAGENO}</strong>页，该页下有<strong>${fn:length(FP_FAMILY_LIST)}</strong>条记录.
								</p>
								<p>
