...
<script type="text/javascript">
		function del(id,name){
			...
			if(result){
				$.ajax({
					url:"../admin/deleteArticleCategory-"+id,
					type:"post",
					dataType:"json",
...
<body>
	<table class="tbl" border="0" cellpadding="0" cellspacing="0">
		<tr>
			...
			<td align="left" valign="middle" class="tbl_middle">帮助/公告--》类型管理</td>
			...
			<td class="context">
				<div class="tbl_div">
					<table border="1" align="center" cellpadding="0" cellspacing="0">
						<tr>
							<td height="30" colspan="6" align="left" valign="middle"><a href="../admin/toAddArticleCategory">点击这里进入添加类型</a></td>
						</tr>
						<tr>
							<td height="30" colspan="6" align="left" valign="middle">筛选:<select onchange="location.href='../admin/articleCategoryList?kind='+this.options[this.selectedIndex].value;">
									<option value="帮助中心" <c:if test="${kind eq '帮助中心' }">selected</c:if>>帮助中心</option>
									<option value="网站公告" <c:if test="${kind eq '网站公告' }">selected</c:if>>网站公告</option>
									<option value="普通文章" <c:if test="${kind eq '普通文章' }">selected</c:if>>普通文章</option>
							</select>
							</td>
						</tr>
						<tr>
							<td width="50" height="30" align="center" valign="middle" bgcolor="#EAEAEA">序号</td>
							<td width="250" height="30" align="center" valign="middle" bgcolor="#EAEAEA">主题</td>
							<td width="120" height="30" align="center" valign="middle" bgcolor="#EAEAEA">种类</td>
							<td width="130" height="30" align="center" valign="middle" bgcolor="#EAEAEA">创建时间</td>
							<td width="100" height="30" align="center" valign="middle" bgcolor="#EAEAEA">操作</td>
						</tr>

						<!--Content Start-->
						<c:forEach var="item" items="${ARTICLE_CATEGORY_LIST}">
							<tr onmouseout="onMouseOut(this)" onmouseover="onMouse(this)">
								...
								<td width="120" height="30" align="center" valign="middle">${item.kind}</td>
								...
								<td width="100" height="30" align="center" valign="middle"><a href="../admin/toEditArticleCategory-${item.id}" class="STYLE11">编辑</a>&nbsp;|&nbsp; <a href="javascript:del(${item.id},'${item.subject}');" class="STYLE10">删除</a></td>
							</tr>
						</c:forEach>
						<!--Content End-->
						...
						<tr>
							<td height="60" colspan="6" align="left" valign="middle"><br />
								<form id="pageForm" action="../admin/articleCategoryList" method="post">
									<input type="hidden" name="sum" id="sum" value="${page.sum}" /> <input type="hidden" name="kind" value="${kind}" /> <input type="hidden" name="count" id="count" value="${page.count}" /> <input type="hidden" name="limit" id="limit" value="${page.limit}" />
									<p>
