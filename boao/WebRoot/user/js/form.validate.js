...
$.formValidator = 
{
	...
	setTipState : function(elem,showclass,showmsg)
	{
		...
		else
		{
			...
			{
				tip.css("display",'').removeClass().addClass( showclass ).html( showmsg );
			}
		}
	},
	...
	pageIsValid : function (validatorGroup)
	{
		...
		else
		{
			...
			if (thefirstid && initConfig.errorFocus) {$("html,body").animate({scrollTop: $("#" + thefirstid).offset().top}, 10);};
		}
	...
	reloadAutoTip : function(validatorGroup)
	{
		...
		$.each(initConfig.validObjects,function()
		{
			if(initConfig.autoTip && !initConfig.tidyMode)
			{
				...
				$("#"+setting.tipID).parent().css("display",'').css({left: x+"px", top: y+"px"});			
			}
		});
	}
};
...
$.fn.formValidator = function(cs) 
{
	...
	return this.each(function(e)
	{
		...
		//初始化显示信息
		if(!initConfig.alertMessage && setting.onShow){
...
$.fn.unFormValidator = function(unbind)
{
	return this.each(function()
	{
		...
		if(unbind){
			...
			$("#"+this.settings[0].tipID).css("display",'');
		}
	});
};
