...
public class DBConnectionPoolMgr {
	...
	synchronized public Connection getFreeConnection() {
		 ...
		 * ，且当前的DBConnection对象的连接记数小于等于最大连接计数 ，则分配此DBConnection连接
		 * 
			...
			if (!model.isUsing()) {
				if (model.getRefNum() >= dbPro.getMaxRefNum()) {
					...
					if (dbPro.isDebug()) {
						...
						System.err.print("In getFreeConnection():model->" + model.toString() + "model.refNum(()" + model.getRefNum());
					}
		...
		if (model == null && this.getOpendConnNum() < dbPro.getMaxConn()) {
			if (dbPro.isDebug()) {
				System.err.println("DBMgr.getFreeConn:opendConnNum" + this.getOpendConnNum());
				System.err.println("DBMgr.getFreeConn:maxConn:" + dbPro.getMaxConn());
			}
	...
	private DBConnection getNewDBConnection() {
		return new DBConnection(dbPro.getClassName(), dbPro.getUrl(), dbPro.getUser(), dbPro.getPassword(), dbPro.isDebug());
