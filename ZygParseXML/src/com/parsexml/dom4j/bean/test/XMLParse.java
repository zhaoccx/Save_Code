...
public class XMLParse {
	...
	protected static Logger log = Logger.getLogger(XMLParse.class.getName());
	...
	public static Map<String, Bean> readXmlFile(String fileName) {
		...
		fileName = Thread.currentThread().getContextClassLoader().getResource(fileName).getPath().substring(1);
		...
		while (beanIterator.hasNext()) {
			...
			while (beanProperties.hasNext()) {
				...
				if (beanProperty.attribute("ref") != null) {
					String[] refString = new String[] { beanProperty.attributeValue("ref") };
				...
				if (beanProperty.element("list") != null) {
					...
					Element listElement = (Element) beanProperty.elements().get(0);
					Iterator<?> valuesIterator = listElement.elements().iterator();
					while (valuesIterator.hasNext()) {
						...
						if (value.getName().equals("ref")) {
							valuesList.add(new String[] { value.attributeValue("bean") });
						}
					}
